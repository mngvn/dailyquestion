...
(function () {
  ...
  // arena with optional gaps on any side; g = {top:[a,b], bottom:[a,b], left:[a,b], right:[a,b]}
  function arena(g, extra) {
    g = g || {};
    const segs = [];
    const horiz = (y, gap) => {
      if (gap) { segs.push([L, y, gap[0], y]); segs.push([gap[1], y, Rg, y]); }
      else segs.push([L, y, Rg, y]);
    };
    const vert = (x, gap) => {
      if (gap) { segs.push([x, T, x, gap[0]]); segs.push([x, gap[1], x, B]); }
      else segs.push([x, T, x, B]);
    };
  ...
  const LEVELS = [
    ...
    {
      name: "The Pillar",
      walls: box(), obstacles: [{ x: 480, y: 300, r: 56 }],
    ...
    {
      name: "Mind the Gap",
      walls: arena({ top: [420, 540] }), obstacles: [],
    ...
    {
      name: "Zigzag",
      walls: arena({ left: [270, 360], right: [270, 360] }, [[330, 120, 330, 360], [630, 240, 630, 480]]),
      obstacles: [],
    ...
    {
      name: "Bumper Room",
      walls: arena({ top: [440, 520], bottom: [440, 520] }),
      obstacles: [{ x: 300, y: 300, r: 46 }, { x: 660, y: 300, r: 46 }],
      enemies: [[140, 140], [820, 140], [140, 460], [820, 460], [480, 110], [480, 490]],
      powerups: [[480, 300], [200, 300]],
      start: [120, 300], budget: 30
    },
    {
      name: "Sliding Doors",
      walls: arena({ left: [250, 350], right: [250, 350] }),
      obstacles: [
        { x: 360, y: 300, r: 34, move: { ax: 0, ay: 1, amp: 150, period: 3 } },
        { x: 600, y: 300, r: 34, move: { ax: 0, ay: 1, amp: 150, period: 3, phase: Math.PI } }
      ],
      enemies: [[140, 140], [820, 140], [140, 460], [820, 460], [480, 120]],
      powerups: [[480, 300]],
      start: [480, 540], budget: 28
    },
    {
      name: "Carousel",
      walls: arena({ top: [200, 300], bottom: [660, 760] }),
      bars: [{ cx: 480, cy: 300, len: 300, ang: 0, spin: 1.1 }],
      obstacles: [],
      enemies: [[140, 150], [820, 150], [140, 450], [820, 450], [480, 90]],
      powerups: [[200, 500], [760, 120]],
      start: [480, 540], budget: 30
    },
    {
      name: "Pinball",
      walls: arena({ top: [80, 170], right: [260, 360] }),
      obstacles: [
      ...
      powerups: [[200, 470], [760, 470]],
      start: [480, 540], budget: 32
    },
    {
      name: "The Gauntlet",
      walls: arena({ top: [440, 520], bottom: [200, 280], left: [270, 350], right: [270, 350] }),
      bars: [
        { cx: 300, cy: 300, len: 200, ang: 0, spin: 1.4 },
        { cx: 660, cy: 300, len: 200, ang: 1, spin: -1.2 }
      ],
      obstacles: [{ x: 480, y: 300, r: 30, move: { ax: 0, ay: 1, amp: 120, period: 2 } }],
      enemies: [[120, 120], [840, 120], [120, 480], [840, 480], [480, 90], [480, 510], [120, 300]],
      powerups: [[200, 200], [760, 400]],
      start: [480, 555], budget: 36
    }
  ];
  ...
  let phase = "intro";
  let levelIndex = 0;
  let walls = box(), obstacles = [], bars = [], liveBars = [], enemies = [], powerups = [], start = [W / 2, H / 2], budget = 0;
  let attemptStart = 0;
  let clock = 0; // seconds, drives moving parts
  ...
  function loadLevel(i) {
    ...
    obstacles = (lv.obstacles || []).map((o) => ({
      x: o.x, y: o.y, r: o.r, cx: o.x, cy: o.y,
  ...
  // update positions of moving obstacles and rotating/translating bars
  ...
  function collide(b) {
    ...
    for (const o of obstacles) {
      let dx = b.x - o.cx, dy = b.y - o.cy, d = Math.hypot(dx, dy);
      const min = R + o.r;
      if (d < min) {
        if (d < 0.01) { dx = 0; dy = -1; d = 1; }
        const nx = dx / d, ny = dy / d;
  ...
  function draw(time) {
    ...
    for (const o of obstacles) {
      ctx.fillStyle = o.move ? "#34305a" : "#262642";
      ctx.beginPath(); ctx.arc(o.cx, o.cy, o.r, 0, 7); ctx.fill();
      ctx.fillStyle = o.move ? "#211d3a" : "#1a1a30";
      ctx.beginPath(); ctx.arc(o.cx, o.cy, o.r - 6, 0, 7); ctx.fill();
    }
    // rotating / sliding bars (a touch brighter so their motion reads)
    ctx.strokeStyle = "#3a3568"; ctx.lineWidth = WALL_T; ctx.lineCap = "round";
    for (const w of liveBars) { ctx.beginPath(); ctx.moveTo(w[0], w[1]); ctx.lineTo(w[2], w[3]); ctx.stroke(); }
  ...
  function frame(now) {
    ...
    if (dt > 1 / 30) dt = 1 / 30;
    clock = now / 1000;
    if (mode === "puzzle") tickMovers(clock); // movers run even while you aim
    step(dt);
    draw(now);
